...
final class RewriteFigureTagsClassTest extends TestCase
{
    ...
    public function test_RewriteClass_1()
    {
        ...
        $this->assertEquals($privateProp1->getValue($tested)[0], 0);
    ...
    public function test_RewriteClass_rewriter($htmlin, $expected)
    {
        ...
        $this->assertInstanceOf('\mvbplugins\fslightbox\RewriteFigureTags', $tested);
        /*
        $out = $tested->rewriteHTML('');
        $this->assertEquals($out, '');

        $out = $tested->rewriteHTML('<body><dib></div</body>');
        $this->assertEquals($out, '<body><dib></div</body>');

        $out = $tested->rewriteHTML('<body><dib><figure><img></img></figure></div</body>');
        $this->assertEquals($out, '<body><dib><figure><img></img></figure></div</body>');
        */
        $result = $tested->rewriteHTML($htmlin);
